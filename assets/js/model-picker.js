 ...
 * sortable popup overlay when the user clicks a model-select trigger button.
 * Columns: Model, Price (in / out per 1M tokens), Context window.
...
(function ($) {
	...
	// Persists across open/close so the user's preferred sort is remembered.
	...
	/**
	 * Sort a model array according to the current sortState.
	 * Deprecated models always sink to the bottom regardless of column.
	 *
	 * @param {Array} arr Model array to sort (not mutated — returns a new array).
	...
	function renderModelRow( model ) {
		...
		if ( isImageModel ) {
			...
			metaRight  = model.description
				? '<span class="ab-mp-ctx" title="Details">' + escHtml( model.description ) + '</span>'
				: '';
		} else {
	...
	function openPicker( $trigger ) {
		...
		var $colHeaders  = $popup.find( '.ab-mp-col-headers' );
		var allModels    = [];
		var currentQuery = '';
		...
		/** Update active-column highlight and sort arrow glyphs. */
		function updateSortHeaders() {
			$colHeaders.find( '.ab-mp-col-header' ).each( function () {
				var col    = $( this ).data( 'col' );
				var $arrow = $( this ).find( '.ab-mp-sort-arrow' );
				if ( col === sortState.col ) {
					$( this ).addClass( 'ab-mp-col-active' );
					$arrow.text( sortState.dir === 'asc' ? ' ▲' : ' ▼' );
				} else {
					$( this ).removeClass( 'ab-mp-col-active' );
					$arrow.text( ' ⇅' );
				}
			} );
		}
		...
		// ── Render ──────────────────────────────────────────────────────
		/** Render filtered + sorted model list. */
		...
		fetchModels( capability ).then( function ( models ) {
			allModels = models;
			renderList( allModels, currentQuery );
		} );

		// ── Column header sort clicks ───────────────────────────────────
		$colHeaders.on( 'click', '.ab-mp-col-header', function () {
			var col = $( this ).data( 'col' );
		...
		$list.on( 'click', '.ab-mp-row', function () {
			...
			for ( var i = 0; i < allModels.length; i++ ) {
				if ( allModels[ i ].id === id ) { model = allModels[ i ]; break; }
			}
			if ( ! model ) return;
